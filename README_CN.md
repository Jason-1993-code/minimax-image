...
# 确认 minimax-image 出现在插件列表中

openclaw plugins inspect minimax-image
# 确认 Capabilities 中包含 image-generation: minimax-image
```
...
用户输入 → LLM 判断需要调用工具 → image_generate 工具
  ...
  → minimax-image.generateImage() 被调用
  → MiniMax API 返回图片 → 用户收到图片
```

关键：`minimax-image`（图像生成）与 `minimax-portal`（文本对话）是**两个独立的 Provider**。图像生成请求需要将 `imageGenerationModel` 指向 `minimax-image`，否则 LLM 不会自动路由到本插件。
...
在 `openclaw.json` 的 `plugins.entries.minimax-image.config` 中配置：
...
| `imageGenerationModel` | **图像生成模型** | `"minimax-image"` |
...
### 插件配置（plugins.entries.minimax-image.config）
...
3. 插件配置 `plugins.entries.minimax-image.config.apiKey`
4. OpenClaw Auth Profile 中的 `minimax-image` API Key 凭证

## 与 minimax-portal 的关系

| | minimax-image（本插件） | minimax-portal（内置） |
|--|------------------------|----------------------|
| **能力** | 图像生成 | 文本对话/LLM |

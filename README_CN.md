...
关键：图像生成 Provider 路由与文本模型路由是分离的。要让图片请求走本插件，需要将 `imageGenerationModel` 指向 `minimax-image-ng`。
...
| 配置项 | 用途 | 示例 |
|--------|------|------|
| `model` | 文本推理模型 | `"minimax/MiniMax-M2.7"` |
...
| `styleWeight` | number | `0.8` | 风格权重（仅 image-01-live，范围 `(0, 1]`） |
...
## 与内置 MiniMax Provider 的定位关系

本插件聚焦于 MiniMax 图像生成能力，强调参数可控和路由可预测。

### `minimax-image-ng` 提供的能力

- Provider ID：`minimax-image-ng`
- 模型：`image-01`、`image-01-live`
- 文生图与图生图（单张 `subject_reference`）
- 两个模型均支持 `aspectRatio`（其中 `21:9` 仅 `image-01`）
- `width`/`height` 仅 `image-01` 支持
- `style` 与 `styleWeight` 仅 `image-01-live` 支持
- 支持 `promptOptimizer`、`aigcWatermark`、`seed`

### 路由说明

- 图像生成由 `imageGenerationModel` 决定具体 Provider。
- 将 `agents.defaults.imageGenerationModel` 设置为 `minimax-image-ng`，即可将图片生成请求路由到本插件。
- 文本模型路由（`model`）与图像生成路由彼此独立。

### 关于内置 MiniMax

内置 Provider 的能力会随 OpenClaw 版本变化。请以 OpenClaw 官方文档中的最新能力矩阵为准。
